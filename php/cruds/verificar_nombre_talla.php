...
try {
  ...
  if (!empty($data['talla'])) {
    $talla = trim($data['talla']); // Limpiar el nombre
    $id = isset($data['id']) ? intval($data['id']) : 0; // ID (opcional)

    // Si se proporciona un ID (edición), excluir ese registro de la validación
    if ($id > 0) {
      $query = "SELECT COUNT(*) FROM tallas WHERE nomtalla = ? AND idtalla != ?";
      $stmt = $dbh->prepare($query);
      $stmt->execute([$talla, $id]);
    } else {
      // Si no hay ID, es un registro nuevo
      $query = "SELECT COUNT(*) FROM tallas WHERE nomtalla = ?";
      $stmt = $dbh->prepare($query);
      $stmt->execute([$talla]);
    }
    ...
    $response["error"] = "Falta el parámetro 'talla'.";
  }
} catch (Exception $e) {
