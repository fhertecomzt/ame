...
try {
  ...
  if (!empty($data['estilo'])) {
    $estilo = trim($data['estilo']); // Limpiar el nombre
    $id = isset($data['id']) ? intval($data['id']) : 0; // ID (opcional)

    // Si se proporciona un ID (edición), excluir ese registro de la validación
    if ($id > 0) {
      $query = "SELECT COUNT(*) FROM estilos WHERE nomestilo = ? AND idestilo != ?";
      $stmt = $dbh->prepare($query);
      $stmt->execute([$estilo, $id]);
    } else {
      // Si no hay ID, es un registro nuevo
      $query = "SELECT COUNT(*) FROM estilos WHERE nomestilo = ?";
      $stmt = $dbh->prepare($query);
      $stmt->execute([$estilo]);
    }
    ...
    $response["error"] = "Falta el parámetro 'estilo'.";
  }
} catch (Exception $e) {
